...
(use-package org
  ...
  (progn
    ...
    (setq org-todo-keywords
          '((sequence "NEXT(n)" "DLGT(l)" "DFER(r)" "WAIT(w)" "|" "DONE(d)" "CNCL(c)")))
            ...
            ("u" "Untagged next actions       "
             ((tags-todo "-{.*}"
                         ((org-agenda-overriding-header "NEXT ACTIONS, no context")
                          (org-agenda-sorting-strategy '(tsia-up))
                          (org-agenda-skip-function '(org-agenda-skip-entry-if 'nottodo '("NEXT")))))))))
    (setq org-agenda-sorting-strategy '(deadline-up todo-state-up tsia-up))
...
; rebind the org-cycle-agenda files
(require 'outline)
(global-unset-key "\C-@")
(define-key outline-mode-map "\C-@" 'org-cycle-agenda-files)
